$(function () {
  ...
  for (let i = 0; i < getAllAddCardItems.length; i++) {
    checkOutHTML += `<div id="addToCardItem">
        ...
        <div>
            <h4>${getAllAddCardItems[i].name}</h4>
            <p class="noPerItem">x${getAllAddCardItems[i].total}</p>
            <p class="AmountPerItem">Amount: Rs <span class="">${
              getAllAddCardItems[i].total * getAllAddCardItems[i].price
            }</span></p>
        </div>
    </div>`;
