      ...
      success: function (res) {
        ...
        if (res) {
          ...
          for (let i = 0; i < res.photos.length; i++) {
            ...
            getImageHTML += `<img class="thumbail ${className}" id="img-${
              res.id + i
            }" src="${res.photos[i]}" alt="Image Preview - ${i}">`;
          }
          let createProductHTML = `<h1 id="name">${res.name}</h1>
                                            <h1 id="brand">${res.brand}</h1>
                                            <h4 class="detailViewHeading">Price: Rs <p id="price">${res.price}</p></h4>
                                            <h4 class="detailViewHeading">Description</h4>
                                            <p id="description">${res.description}</p>
                                            <h4 class="detailViewHeading">Product Preview</h4>
                                            <div class="previewImage">${getImageHTML}</div>
                                            <div ><button id="addToCart">Add to Cart</button></div>`;

          productDetailWrapper.insertAdjacentHTML(
            "beforeEnd",
            createProductHTML
          );
          ...
          for (let i = 0; i < getAllImageTag.length; i++) {
            getAllImageTag[i].addEventListener("click", function (e) {
              let getActiveElem = document.querySelector(
                ".previewImage .active"
              );
          ...
          $("#addToCart").click(function () {
            ...
            localStorage.setItem(
              "addToCart",
              JSON.stringify(getAllAddToCartItem)
            );
            addToCardCount();
          });
        }
